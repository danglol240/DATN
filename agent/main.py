...
def send_event(backend_url, agent_id, etype, data):
    try:
        requests.post(f"{backend_url}/api/events",
...
def execute_command_logic(backend_url, agent_id, cmd):
    try:
        ...
        cmd_id = cmd.get('id')
        logging.info(f"[WS CMD] Nhận lệnh: {action} | params: {params}")
        ...
        if action == 'block_ip':
            ...
            if ip:
                success = block_ip(ip)
                if success:
                    send_event(backend_url, agent_id, 'Network', {"action": "blocked_ip", "connection": {"remote_ip": ip, "remote_port": "manual"}})
            ...
            success = add_custom_rule(chain, protocol, port, target, log, states, 
            ...
            if num:
                ...
                if success:
                    send_event(backend_url, agent_id, 'Network', {"action": "deleted_rule", "chain": chain, "num": num})
            else:
                protocol = params.get('protocol', 'tcp')
                port = params.get('port')
                target = params.get('target', 'ACCEPT')
                if port:
                    ...
                    if success:
                        send_event(backend_url, agent_id, 'Network', {"action": "deleted_rule", "chain": chain, "protocol": protocol, "port": port, "target": target})

        status_val = "done" if success else "failed"
        requests.patch(f"{backend_url}/api/commands/{cmd_id}/done", json={"status": status_val}, timeout=5)
        logging.info(f"[WS CMD] Lenh {action} {'thanh cong' if success else 'that bai'}")
    except Exception as e:
        logging.error(f"[WS CMD] Loi thuc thi lenh: {e}")
...
def main():
    ...
    logging.info(f"== Agent Khoi Dong | ID: {agent_id} ==")
    ...
    def new_command(data):
        logging.info(f"WebSocket: Nhận sự kiện new_command")
        try:
            ...
            logging.error(f"Lỗi khi pull lệnh sau thông báo WS: {e}")
        ...
        while True:
            try:
                if not sio.connected:
                    sio.connect(b_url, wait_timeout=10)
                    sio.wait()
            except Exception as e:
    ...
    while True:
        ...
        try:
            ...
            for conn in suspicious_conns:
                ...
                logging.warning(f"Phat hien ket noi nghi ngo toi {remote_ip}:{conn['remote_port']}")
                if block_ip(remote_ip):
                    send_event(backend_url, agent_id, 'Network', {"action": "blocked_ip", "connection": conn})

        except requests.exceptions.RequestException as e:
            logging.error(f"Loi ket noi Backend: {e}")
        except Exception as e:
            logging.error(f"Loi noi bo Agent: {e}")
